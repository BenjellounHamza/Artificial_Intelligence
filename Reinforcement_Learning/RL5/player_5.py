...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        epsilon_t = ScheduleLinear(anneal_timesteps, epsilon_final, epsilon_initial).value(current_total_steps)
...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        Q = np.array([np.array([random.random() for j in range(na)]) for _ in range(ns)])
        # ADD YOUR CODE SNIPPET BETWEEN EX. 2.1
        ...
        while episode <= self.episode_max and self.threshold < diff:
